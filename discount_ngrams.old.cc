...
struct DiscountParameters {
  float D; // absolute discounting.
  float tau; 
  DiscountParameters(): D(0.0), tau(0.0) {}
};
...
double apply_discount(double *count, float D, float tau) {
  if (*count <= D) {
    double ans = *count;
    *count = 0;
    return ans;
    ...
    *count -= D;
    double tau_discount = *count * tau / (*count + tau);
    *count -= tau_discount;
...
void read_parameters(const char *config_file,
  ...
  int n;
  while ((n=fscanf(f, "D=%f tau=%f\n", &params.D, &params.tau)) == 2) {
    v->push_back(params);
    fprintf(stderr, "discount_ngrams: for n-gram order %d, D=%f, tau=%f\n", (int)v->size(),
            params.D, params.tau);
    assert(params.D >= 0.0 && params.tau >= 0.0);
  }
  if (n != EOF) {
...
void apply_discounts(int h) {
  ...
  float D = discount_params[h].D, tau = discount_params[h].tau;
  unordered_map<string, double> &counts = ngram_counts[h].counts;
  ...
  for (unordered_map<string, double>::iterator iter = counts.begin();
    ...
    double discounted = apply_discount(&count, D, tau);
